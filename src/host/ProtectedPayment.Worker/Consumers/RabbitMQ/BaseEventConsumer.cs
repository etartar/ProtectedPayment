    ...
    IServiceProvider serviceProvider) : BackgroundService where TEvent : BaseEvent
{
    ...
    private async Task Consumer_ReceivedAsync(object sender, BasicDeliverEventArgs @event)
    {
        try
        {
            ...
            bool hasIdempotency = await inboxMessageService.HasIdempotencyKey(idempotencyKey, eventType);
            ...
            await inboxMessageService.SaveInboxMessage(idempotencyKey, eventType, eventData);
