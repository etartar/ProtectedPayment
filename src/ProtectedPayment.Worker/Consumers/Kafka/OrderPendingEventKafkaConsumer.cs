...
internal sealed class OrderPendingEventKafkaConsumer : BaseKafkaEventConsumer<OrderPendingEvent>
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                {
                    bool commitMessage = await HandleMessageAsync(consumeResult);
    ...
    private async Task<bool> HandleMessageAsync(ConsumeResult<Guid, OrderPendingEvent> consumeResult)
    {
